/*
  # Fresh Start - Complete USA Filmhub Database Schema

  1. New Tables
    - `users`
      - Custom user profiles (extends auth.users)
      - Fields: id, email, name, role (admin/filmmaker/partner), created_at
    - `content`
      - Movies, series, and episodes
      - Fields: id, title, type, status, owner_id, owner_email, metadata, created_at
    - `payment_statements`
      - Payment records uploaded by admin
      - Fields: id, filmmaker_id, period_start, period_end, amount, file_url, uploaded_by, created_at
    - `title_selections`
      - Partner selections of titles
      - Fields: id, partner_id, content_id, selected_at
    - `analytics`
      - Usage and viewing statistics
      - Fields: id, content_id, views, revenue, period, created_at

  2. Security
    - Enable RLS on all tables
    - Admin access: nancycriss@yahoo.com, sherri@olivebranch.tv, nancy@olivebranch.tv, info@olivebranchfilmstudios.com
    - Filmmakers: can only see/edit their own content and payments
    - Partners: can only see approved content and their selections
    - All policies check authentication and role-based access

  3. Trigger Functions
    - Auto-create user profile on signup
    - Bypass RLS for trigger functions using security definer

  4. Important Notes
    - Uses auth.uid() for all user identification
    - No destructive operations (all tables created with IF NOT EXISTS)
    - Comprehensive RLS policies for multi-role access control
*/
